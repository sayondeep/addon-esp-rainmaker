...
async def _discover_and_connect(nodeid, pop, transport_type, port, node_cache):
    ...
    transport_obj, security_obj = await _fresh_establish(nodeid, '', transport_type, 0, port)
    if transport_obj is not None and security_obj is not None:
        if await _probe_session(transport_obj, security_obj):
    ...
    if pop:
        transport_obj, security_obj = await _fresh_establish(nodeid, pop, transport_type, 1, port)
        if transport_obj is not None and security_obj is not None:
    ...
    log.debug(f"Node {nodeid} does not appear to support local control")
    if node_cache:
        node_cache.set_local_control_capability(nodeid, {
            'supported': False,
        })
        ...
        if not resumed:
            if not explicit_sec_ver and not pop and node_cache:
                ...
                if cap is None:
                    ...
                    transport_obj, security_obj = await _fresh_establish(
